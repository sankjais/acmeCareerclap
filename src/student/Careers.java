...
public class Careers {
	...
	public static void careerTest(TestData student)

	{
						...
						if (driver.getTitle().contains("CareerClap")) {
							...
							if (driver.getCurrentUrl().equals(student.landingURL)) {
								if (driver.getTitle().contains("CareerClap")) {
									...
									driver.findElement(By.xpath("//div[11]/span")).click();
									Thread.sleep(2000);
									wait.until(ExpectedConditions.urlToBe(student.careers));
									if (driver.getCurrentUrl().equals(student.careers)) {
										if (driver.getTitle().contains("CareerClap")) {
											System.out.println("career blog page is loaded successful");
											Thread.sleep(2000);
		...
		InterruptedException e) {
			...
			if (flag == 0) {
				System.out.println(
						"total time for cheking career blogs = " + (endtime - starttime) / 6000 + " SECONDS");
			} else {
				System.out.println("total time for checking career blogs but fail's is = " + (endtime - starttime) / 6000
						+ " SECONDS");
